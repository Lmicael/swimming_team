...
class _GraphicScreenState extends State<ComparisonChart> {
    ...
    try {
      ...
      for (QueryDocumentSnapshot document in querySnapshot.docs) {
        ...
        for (QueryDocumentSnapshot subDocument in subCollection.docs) {
          ...
          uniqueTempos.addAll(tempoList
              .where((tempo) => tempo is num) // Verifica se é um número
              .map((tempo) =>
                  (tempo as num).toDouble())); // Converte para double
        }
      }
  ...
  Future<void> TempoValues(String id) async {
    try {
      QuerySnapshot querySnapshot =
          await _firestore.collection('treinos').get();
      Set<double> uniqueTempos = Set();
      ...
      for (QueryDocumentSnapshot document in querySnapshot.docs) {
        ...
        for (QueryDocumentSnapshot subDocument in subCollection.docs) {
          List<dynamic> tempoList = subDocument['tempos'];
          uniqueTempos.addAll(tempoList
              .where((tempo) => tempo is num)
              .map((tempo) => (tempo as num).toDouble()));
        }
      }

      setState(() {
        Tempos = List.from(uniqueTempos);
        print(Tempos);
        atletas = [Atleta(allTempos)]; // Use 'allTempos' em vez de 'Tempos'
      });
    } catch (e) {
      print('Erro ao obter valores de tempo: $e');
      throw e;
      ...
      body: Padding(
        ...
        child: Column(
          ...
          children: [
            ...
            ElevatedButton(
              onPressed: () async {
                String codigoAtleta = _codigoAtletaController.text;
                await TempoValues(codigoAtleta);
                setState(() {});
              },
